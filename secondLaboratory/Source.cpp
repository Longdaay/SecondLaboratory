...
///			9)  заменит каждый элемент массива суммой значений текущего элементы и следующего (последний = последний + первый). Сделает «обратную» сортировку – каждому элементу присвоить случайный номер в массиве;
//			8)	выводит скорость обмена элементов массива, номера пользователем.
...
int main()
{
	...
	time_span = static_cast<chrono::duration<double>>(end - start);   // высчитываем время, затраченное на событие
	summ_time = summ_time + time_span.count();
	cout << "Количество элементов равных среднему значению ("<< middle << "): " << count << endl;
	...
	print_array(digits_array);
	time_span = static_cast<chrono::duration<double>>(end - start);   // высчитываем время, затраченное на событие
	cout << endl << "Время, затраченное на обмен: " << time_span.count() << " секунд" << endl << endl;
	return 0;
}