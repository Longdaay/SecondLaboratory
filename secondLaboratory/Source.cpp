...
///			8)  заменит каждый элемент массива суммой значений текущего элемента и следующего (последний = последний + первый). Сделает «обратную» сортировку – каждому элементу присвоить случайный номер в массиве;
...
void insertion_sort(vector<int>& digits_array)
{
	...
	for (int i = 1; i < digits_array.size(); i++)
	{
		...
		while (j >= 0 and digits_array[j] > min_value) //пока текущий элемент меньше предыдущего и j не присвоилось
...
int main()
{
	...
	cout << "Для поиска значения нам необходимо снова отсортировать массив" << endl;
